...
NumericalVelocityProfile::NumericalVelocityProfile(double v_start, double v_end,
...
double NumericalVelocityProfile::maxVelocityCurve(double& c) {
    return v_maxrot_ / std::max(c, 1e-6);
}
...
double NumericalVelocityProfile::isolatedConstraints(double& c) {
    ...
    return std::min({v_maxtrans_, v_maxc, v_maxf});
}
