...
int main(int argc, char** argv) {
    ...
    while (ros::ok() && !first) {
        ...
        path_msg.poses[1].pose.position.x = 2.0;
        path_msg.poses[1].pose.position.y = 2.0;

        path_msg.poses[2].pose.position.x = 2.365;
        path_msg.poses[2].pose.position.y = 2.2;

        // path_msg.poses[0].pose.position.x = -1.365;
        // path_msg.poses[0].pose.position.y = 1.8;
        // path_msg.poses[0].pose.orientation = tf::createQuaternionMsgFromYaw(0.0);
    
        // path_msg.poses[1].pose.position.x = 2.365;
        // path_msg.poses[1].pose.position.y = 1.8;
